    ...
    for (const key in data) {
      ...
      if (
        ...
        if (fileData.includes("use client")) {
          ...
          routes.push(
            `<Route ${
              subPaths[subPaths.length - 1] === parentPath || routePath === "" || routePath === "/src/"
                ? "index"
                : addPath
            }  element={
          ...
          if (window !== undefined) {
            function getElementAttributeByPrefix(documentId, prefix) {
              ...
              for (const attribute of element.attributes) {
                ...
                // Check if attribute name starts with the prefix
              ...
              // No attribute found with the prefix
              return toReturn;
            }
            ...
            // const data  = getElementAttributeByPrefix("root","data-server-prop")
            // console.log({data})
...
const createApp = (cwd,debug=false) => {
  try {
  ...
  fs.mkdirSync(path.resolve(cwd, "__build__"), { recursive: true });

  fs.writeFileSync(
    path.resolve(cwd, "__build__", "app.jsx"),
    createReactAppWithRoutes(
      findJSXFiles(path.resolve(cwd, "src", "app"))
    )
  );
  fs.writeFileSync(
    path.resolve(cwd, "__build__", "BrowserRouterWrapper.jsx"),
    generateBrowserRouterWrapper()
  );
  fs.writeFileSync(
    path.resolve(cwd, "__build__", "StaticRouterWrapper.jsx"),
    generateStaticRouterWrapper()
  );
  fs.writeFileSync(
    path.resolve(cwd, "__build__", "main.jsx"),
  ...
  if(fs.existsSync(path.resolve(cwd, "src","global.css"))){
    return [path.resolve(cwd, "build", "app", "main.js"),path.resolve(cwd, "src", "global.css")] 
  }
  return path.resolve(cwd, "build", "app", "main.js");
};
